...
function Cart({ carts, setCarts }) {
    ...
    const totalPrice = carts.reduce((total, product) => total + product.price * product.amount, 0);

    const totalAmount = carts.reduce((total, product) => total + product.amount, 0);
    ...
    const handleSell = (e) => {
        const existingProductSell = carts.find((product) => product.name === e.name);
                ...
                {carts.map((e, i) => (
                    <div className='cart_user' key={i}>
                        ...
                        <span
                            ...
                            className="fa-solid fa-minus"></span>
                        <p>{e.price * e.amount}</p>

                        <span
                            onClick={() => handleDelete(i)}
                            className="fa-solid fa-trash"></span>
                    </div>
                ))}
                <div className='total_cart'>
                    <p >Total Carts: {totalPrice} </p>
                    <div className='nav_cart'>
                        <button
                            onClick={handlePay}
                        >Pay Carts<span className="fa-solid fa-sack-dollar"></span></button>
                        <button
                            onClick={handleClearCart}
                        >Clear Carts<span className="fa-solid fa-trash"></span></button>
                    </div>
                </div>

            </div>
                    {/* huhu chia hổng có kịp hiu hiu  khóc qtqd*/}
            <div className={hideCart2 ? 'cart_2' : 'none'}>
                <div className='info'>

                    {carts.map((e) => (
                        <div className='flex'>
                            <div> <span>{e.amount} x</span> <span>{e.name}</span></div>
                            <p>{e.price * e.amount}$</p>
                        </div>
                    ))}
                ...
                <div>
                    ...
                    <div>Payment </div>
                    <div>Total Pay: {totalPrice}$</div>
                </div>
                <div className='button'>
                    <button onClick={handlePayCart2}>PAY</button>
                    <button onClick={handlePayCart2}>Cancel</button>
                </div>
            </div>
